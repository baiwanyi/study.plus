...
// 专属测验：一次性出 20 道题（简答 10 题固定 + 单选/多选 10 题自由分配，总分 100 分自动分配）
export const promptStudynotesQuizGenerate =
    '你是中小学「学习辅导员」，请基于学生刚写好的学习心得，一次性出20道巩固测验题。\n' +
    '学科：{subject}，课题：{topic}\n' +
    '学生心得内容：\n' +
    '- 一句话概括：{summary}\n' +
    '- 自己的例子：{example}\n' +
    ...
    '4. 难度从基础起步、逐步加深：前5题考查核心知识的准确理解，中间5题考查能否把概括与自己的例子联系起来灵活运用，后10题针对卡壳点设综合应用与辨析陷阱。\n' +
    '5. 每道题只问一个明确的问题，表述口语化、贴近学生，像真人辅导员一样（不要说教）。\n' +
    '6. 题目必须是"能独立作答"的问句，不要依赖对话上下文。\n' +
    '7. 不要给出答案，只出题。\n' +
    '\n' +
    '题型要求（必须严格遵守）：\n' +
    '- 共 20 题，index 从 1 到 20 连续。\n' +
    '- 其中 10 题为简答题（type="essay"），考查理解、解释、举例与迁移运用。\n' +
    '- 另外 10 题为客观题：单选题（type="single"）和多选题（type="multi"）自由分配，但每种至少 1 题。\n' +
    '- 客观题必须包含 options 数组（3~4 个选项），answer 为选项字母；多选题 answer 用逗号分隔且按字母升序（如 "A,C"）。\n' +
    '- 简答题没有 options 字段，answer 为要点式参考答案。\n' +
    '- 每道题必须包含 points 字段（正整数，表示该题分值）；20 道题的 points 之和必须精确等于 100。\n' +
    '- 分值建议：简答题分值较高（如每题 8~12 分），单选题分值较低（如每题 3~6 分），多选题分值居中（如每题 4~8 分），总和必须为 100。\n' +
    ...
    '    { "index": 11, "type": "essay", "question": "第11题内容", "answer": "参考答案要点", "points": 10 }\n' +
    '  ]\n' +
    '}\n' +
    '（共20题，index 从 1 到 20 连续；20 道题的 points 之和必须为 100）\n'
