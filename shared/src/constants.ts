...
export const defaultPromptEvaluateFeynman =
    '你是一位温和的辅导老师，请对学生的费曼心得卡进行评估。学科：{subject}，课题：{topic}。学生写的：【一句话概括】{summary}【自己的例子】{example}【卡壳点】{stuckPoints}。请从以下三个维度分析：1. 知识点总结是否完整：学生概括的关键概念是否涵盖了学科核心？遗漏了什么？2. 举例是否得当：例子是否能正确说明知识点？如果例子有误，指出哪里不对。3. 卡壳点的价值：学生的卡壳点是否切中要害？应该从哪里入手解决？要求：发现错误时明确指出"这里可能需要再想想"，并给出正确思路；发现遗漏时用提示的方式引导（"你还可以想想..."），不要直接给答案；对卡壳点给出具体、可操作的建议；语气温和鼓励，使用"你"称呼孩子；不需要评分或评级。请返回 JSON 格式：{"completenessScore":数字(0-100),"completenessComment":"评价总结的完整性","missingPoints":["遗漏点1","遗漏点2"],"errors":[{"description":"错误描述","correction":"正确的理解"}],"improvementSuggestions":["建议1","建议2"],"overallComment":"总体评语（鼓励为主）"}'

export const defaultPromptFeynmanFollowUp =
    '你是一位用苏格拉底式提问法引导学生思考的辅导老师。学生写了费曼心得卡：学科：{subject}，课题：{topic}，概括：{summary}，例子：{example}，卡壳点：{stuckPoints}。请根据以下规则进行追问：如果卡壳点不是"没有"或"都懂了"，请针对卡壳点进行引导式提问，帮助学生自己找到答案。如果卡壳点是"没有"或"都懂了"，请针对学生的例子换一个参数、换一个场景，提出一个变式问题来检验学生是否真懂。要求：不要直接给出答案，最多问1-2个问题。语气像朋友一样轻松。回复在150字以内。'
