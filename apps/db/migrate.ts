...
async function migrate(): Promise<void> {
  ...
  const submissionsExists = await client.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='submissions'");
  if (submissionsExists.rows.length > 0) {
    ...
    try {
      ...
      await client.execute("SELECT grade FROM submissions WHERE grade = 'E' LIMIT 1");
    } catch {
      ...
      await client.execute('INSERT INTO submissions (id, task_id, content, grade, ai_score, created_at) SELECT id, task_id, content, grade, ai_score, created_at FROM submissions_old');
  ...
  const exchangesExists = await client.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='exchanges'");
  if (exchangesExists.rows.length > 0) {
    ...
    try {
      ...
      const testResult = await client.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='exchanges'");
      const tableSql = testResult.rows[0]?.sql as string | undefined;
      if (tableSql && tableSql.includes("item_type") && tableSql.includes("CHECK")) {
        console.log('Migrating exchanges table to support dynamic item_type...');
        await client.execute('ALTER TABLE exchanges RENAME TO exchanges_old');
        await client.execute(`
        ...
        `);
        await client.execute('INSERT INTO exchanges SELECT * FROM exchanges_old');
  ...
  try {
    await client.execute("ALTER TABLE submissions ADD COLUMN scored_at TEXT");
  ...
  try {
    await client.execute("ALTER TABLE ai_usage_logs ADD COLUMN task_title TEXT");
  ...
  try {
    await client.execute("ALTER TABLE ai_usage_logs ADD COLUMN task_id INTEGER");
  ...
  const oldDefault = await client.execute("SELECT value FROM rule_config WHERE key = 'default'");
  if (oldDefault.rows.length > 0) {
    try {
      ...
      if (homework) separateKeys.push({ key: 'homework', value: homework });
      ...
      if (Object.keys(examData).length > 0) separateKeys.push({ key: 'exam', value: examData });

      // exchange
      const exchange = src?.exchangeRates ?? src?.exchange;
      if (exchange) separateKeys.push({ key: 'exchange', value: exchange });
      ...
      if (src?.monthlyBasePoints) system.monthlyBasePoints = src.monthlyBasePoints;
      else if (src?.exam?.basePoints) system.monthlyBasePoints = src.exam.basePoints;
      if (src?.minimumPointsForPrivileges) system.minimumPointsForPrivileges = src.minimumPointsForPrivileges;
      else if (src?.exam?.privilegeMinPoints) system.minimumPointsForPrivileges = src.exam.privilegeMinPoints;
      if (Object.keys(system).length > 0) separateKeys.push({ key: 'system', value: system });

      for (const item of separateKeys) {
        const existing = await client.execute({ sql: "SELECT id FROM rule_config WHERE key = ?", args: [item.key] });
      ...
      await client.execute("DELETE FROM rule_config WHERE key = 'default'");
      console.log('Migrated rules from single default key to separate keys.');
    } catch (e) {
      console.log('Failed to parse old default rules, inserting new separate keys instead.');
    }
  }
  ...
  const defaultRules: { key: string; value: string }[] = [
    ...
    {
      key: 'system',
      value: JSON.stringify({ monthlyBasePoints: 500, minimumPointsForPrivileges: 100 }),
    },
  ];

  for (const rule of defaultRules) {
    const existing = await client.execute({ sql: "SELECT id FROM rule_config WHERE key = ?", args: [rule.key] });
    if (existing.rows.length === 0) {
      await client.execute({
        sql: "INSERT INTO rule_config (key, value) VALUES (?, ?)",
  ...
  try {
    const examRow = await client.execute({ sql: "SELECT value FROM rule_config WHERE key = 'exam'", args: [] });
    if (examRow.rows.length > 0) {
      const examVal = JSON.parse(examRow.rows[0].value as string) as Record<string, unknown>;
      if (examVal.basePoints !== undefined || examVal.privilegeMinPoints !== undefined) {
        // Move to system key
        const systemRow = await client.execute({ sql: "SELECT value FROM rule_config WHERE key = 'system'", args: [] });
        let systemVal: Record<string, unknown> = {};
        if (systemRow.rows.length > 0) {
          systemVal = JSON.parse(systemRow.rows[0].value as string) as Record<string, unknown>;
        }
        if (systemVal.monthlyBasePoints === undefined && examVal.basePoints !== undefined) {
          systemVal.monthlyBasePoints = examVal.basePoints;
        }
        if (systemVal.minimumPointsForPrivileges === undefined && examVal.privilegeMinPoints !== undefined) {
          systemVal.minimumPointsForPrivileges = examVal.privilegeMinPoints;
        }
        // Remove old fields from exam
        delete examVal.basePoints;
        delete examVal.privilegeMinPoints;
        await client.execute({ sql: "UPDATE rule_config SET value = ? WHERE key = 'exam'", args: [JSON.stringify(examVal)] });
        if (systemRow.rows.length > 0) {
          await client.execute({ sql: "UPDATE rule_config SET value = ? WHERE key = 'system'", args: [JSON.stringify(systemVal)] });
        } else {
          await client.execute({ sql: "INSERT INTO rule_config (key, value) VALUES ('system', ?)", args: [JSON.stringify(systemVal)] });
        }
        console.log('Migrated basePoints/privilegeMinPoints from exam to system key.');
      }
    }
  } catch (e) {
    console.log('Exam→system key migration skipped or already done:', (e as Error).message);
  }
