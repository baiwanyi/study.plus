...
import { exchanges, pointRecords, monthSummary } from '@apps/db/schema'
import { eq, desc, and, gte, lte } from 'drizzle-orm'
import {
    loadRulesWithSrc,
    getExchangeItemLabel,
} from '@apps/routes/rules-loader'
    ...
    const records: Exchange[] =
        conditions.length > 0
            ? ((await db
                  ...
                  .orderBy(desc(exchanges.createdAt))) as Exchange[])
            : ((await db
                  .select()
                  .from(exchanges)
                  .orderBy(desc(exchanges.createdAt))) as Exchange[])
...
router.post(
    ...
    async (
        ...
        if (!itemType || !pointsCost) {
            res.status(400).json({
                error: 'itemType and pointsCost are required',
            })
            return
        }
...
router.post(
    ...
    async (
        ...
        // Update exchange status and refund points to available balance
        try {
            const currentMonth: string = new Date().toISOString().slice(0, 7)
            ...
            // 将原始兑换扣减记录的 relatedType 标记为 revoked（以便统计时排除）
            await db
                .update(pointRecords)
            ...
            // Record refund for history tracking (type: earn for tracking only, not counted in totals)
            await db.insert(pointRecords).values({
                type: 'earn',
