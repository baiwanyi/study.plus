...
export async function createMockDb(tmpDirPrefix: string): Promise<{
    ...
    // Create current month summary (needed for exchange balance calculation)
    // basePoints 600 ensures availableBalance = 600 - 500 (monthlyBasePoints) = 100 > 0
    const currentMonth = new Date().toISOString().slice(0, 7)
    await client.execute({
        sql: 'INSERT OR IGNORE INTO month_summary (month, base_points, balance) VALUES (?, 600, 600)',
        args: [currentMonth],
    })
