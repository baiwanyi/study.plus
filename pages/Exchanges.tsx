...
import { exchangesApi, pointsApi, optionsAPI } from '@apps/lib/api'
import type { Exchange, MonthSummary, ExchangeItemRule } from '@apps/lib/types'
import { formatErrorMessage, isAdmin } from '@apps/lib/utils'
import { useSnackbar } from '@apps/components/Snackbar'
import Modal from '@apps/components/Modal'
import ExchangesStatsCards from '@layout/ExchangesStatsCards'
        ...
        try {
            const [exchangesData, summaryData, exchangeRulesData] =
                await Promise.all([
                    exchangesApi.list(),
                    pointsApi.summary(),
                    optionsAPI.get('exchange').catch(() => null),
                ])
            // 只显示活跃的兑换记录（排除已撤销的）
            setExchanges(exchangesData.filter((e) => e.status === 'active'))
            setSummary(summaryData)
            if (exchangeRulesData) {
                setExchangeRules(parseExchangeData(exchangeRulesData))
            }
        } catch (err) {
    ...
    return (
        <>
            <div className="space-y-6">
                ...
                <ExchangesStatsCards summary={summary} />
