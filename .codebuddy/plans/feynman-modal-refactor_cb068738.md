...
2. **编辑心得卡**：在查看器中点击编辑，或直接（未来扩展）打开编辑器 Modal（带预填数据）
3. **查看/评估心得卡**：点击卡片弹出 Modal 详情视图（size="xl"），包含心得内容展示、AI 评估结果（环形评分图）、AI 评估/重新评估按钮、AI 追问按钮
4. **删除心得卡**：在卡片列表项上显示删除按钮，仅管理员（isAdmin()）可见，点击触发确认后执行删除
